...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'PUT') { // Anda dapat menggunakan PUT atau PATCH sesuai preferensi Anda
    try {
      const { id, jenis_simpanan, deskripsi, amount, created_by, created_at } = req.body;
      ...
      const query = `
        UPDATE jenissimpanans
        SET jenis_simpanan = ?, deskripsi = ?, amount = ?, created_by = ?, created_at = ?
        WHERE id = ?
      `;
      await connection.execute(query, [jenis_simpanan, deskripsi, amount, created_by, created_at, id]);
